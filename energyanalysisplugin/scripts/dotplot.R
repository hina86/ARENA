...
graph <- ggdotplot(my_data, x = xProp, y = yProp, 
                   fill = xProp, color = xProp,
                   palette = colorArr,
                   ylim = c(min, max),
                   size = 0.5,
                   order = orderArr, 
...
print("gg")
finalGraph <- graph +  #guides(col=guide_legend(xLabel), size=guide_legend(yLabel))+
  labs(caption = cap) + 
